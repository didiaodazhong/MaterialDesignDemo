...
/**
 * Created by peixing on 2017/3/1.
 */
...
public class MiClockView extends View {
    ...
    public MiClockView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBackgroundColor = ta.getColor(R.styleable.MiClockView_backgroundColor, 0x237EAD);
        setBackgroundColor(mBackgroundColor);
        mLightColor = ta.getColor(R.styleable.MiClockView_lightColor, 0xffffff);
        mDarkCOlor = ta.getColor(R.styleable.MiClockView_darkColor, 0x80ffffff);
        mTextSize = ta.getDimension(R.styleable.MiClockView_textSize, DensityUtils.dp2px(context, 14));
        ta.recycle();
...
//        return super.onTouchEvent(event);
    ...
    private void getCanvasTranslate(MotionEvent event) {
        float translateX = -(event.getY() - getHeight() / 2);
        float translateY = event.getX() - getWidth() / 2;
        float[] percentArc = getPercent(translateX, translateY);
        mCanvasTranslateX = percentArc[0] * mCanvasTranslateX;
        mCanvasTranslateY = percentArc[1] * mCanvasTranslateY;
    }
    ...
    private void getCameraRotate(MotionEvent event) {
        ...
        float[] percentArc = getPercent(rotateX, rotateY);
        mCameraRotateX = percentArc[0] * mMaxCameraRotate;
        mCameraRotateY = percentArc[1] * mMaxCameraRotate;
...
//        Log.i(TAG, "getTimeDegree: " + calendar.get(Calendar.HOUR) + "==" + calendar.getTime());
    ...
    private void drawMinuteHand() {
        ...
        mMinuteHandPath.moveTo(getWidth() / 2 - 0.01f * mRadius,
                getHeight() / 2 - 0.03f * mRadius
        );
        mMinuteHandPath.lineTo(getWidth() / 2 - 0.008f * mRadius, offset + 0.365f * mRadius);
        mMinuteHandPath.quadTo(getWidth() / 2,
                offset + 0.345f * mRadius,
                getWidth() / 2 + 0.008f * mRadius,
        ...
        mCircleRectF.set(getWidth() / 2 - 0.03f * mRadius,
                getHeight() / 2 - 0.03f * mRadius,
                getWidth() / 2 + 0.03f * mRadius,
                getHeight() / 2 + 0.03f * mRadius
        );
    ...
    private void drawHourHand() {
        ...
        mHourHandPath.reset();
        float offset = mPaddingTop + mTextRect.height() / 2;
        mHourHandPath.moveTo(getWidth() / 2 - 0.018f * mRadius,
                getHeight() / 2 - 0.03f * mRadius);
        mHourHandPath.lineTo(getWidth() / 2 - 0.009f * mRadius, offset + 0.48f * mRadius);
        mHourHandPath.quadTo(
                getWidth() / 2,
                offset + 0.46f * mRadius,
                getWidth() / 2 + 0.009f * mRadius,
                offset + 0.48f * mRadius
        );
        mHourHandPath.lineTo(getWidth() / 2 + 0.018f * mRadius,
                getHeight() / 2 - 0.03f * mRadius);
        mHourHandPath.close();
        mHourHandPaint.setStyle(Paint.Style.FILL);
    ...
    private void drawTimeText() {
        ...
        mCanvas.drawText("12",
                getWidth() / 2 - textLargeWidth / 2,
                mPaddingTop + mTextRect.height(),
                mTextPaint);
        timeText = "3";
        mTextPaint.getTextBounds(timeText, 0, timeText.length(), mTextRect);
        //一位数字的宽
        ...
        mCanvas.drawText("3",
                getWidth() - mPaddingRight - mTextRect.height() / 2 - textSmallWidth / 2,
                getHeight() / 2 + mTextRect.height() / 2,
                mTextPaint);
        mCanvas.drawText("6",
                getWidth() / 2 - textSmallWidth / 2,
                getHeight() - mPaddingBottom,
                mTextPaint);
        mCanvas.drawText("9",
                mPaddingLeft + mTextRect.height() / 2 - textSmallWidth / 2,
                getHeight() / 2 + mTextRect.height() / 2,
                mTextPaint);
        //画四个弧
